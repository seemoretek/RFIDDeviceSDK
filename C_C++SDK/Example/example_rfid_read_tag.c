/**
 * @file example_rfid_read_tag.c
 * @brief C Example：RFID Tag Read
 *
 * Compile:
 *   gcc -o example_rfid_read_tag example_rfid_read_tag.c -ldevice_api
 */
...
int main(int argc, char* argv[]) {
    ...
    /* Select tag before read */
    printf("Do you want select tag before read? Y/N");
    char resp[16];
    scanf("%s", resp);
    ...
    rfid_read_t read = {0};
    read.mask =  0;
    read.mask |= RFID_TAGOPS_MASK_ENABLE_TIMEOUT;
    read.timeout = 2000;

    if (select_en) {
        read.mask |= RFID_TAGOPS_MASK_ENABLE_SELECT;
        read.select_en = SELECT_MASK_1;
    }

    read.bank = UHF_BANK_USER;
    read.wpoint = 0;
    read.wlength = 4;

    status = device_rfid_read_tag(dev, &read);
    if (status != DEVICE_OK) {
        fprintf(stderr, "Failed to read tag: 0x%02X\n", (int)status);
        device_destroy(dev);
        return 1;
    }
    printf("Tag read successfully!\n");
    printf("Tag data: ");
    for (int i = 0; i < read.wlength * 2; i++) {
