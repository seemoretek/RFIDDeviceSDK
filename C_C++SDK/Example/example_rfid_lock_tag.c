/**
 * @file example_rfid_lock_tag.c
 * @brief C Example：RFID Tag Lock
 *
 * Compile:
 *   gcc -o example_rfid_lock_tag example_rfid_lock_tag.c -ldevice_api
 */
...
int main(int argc, char* argv[]) {
    ...
    /* Select tag before lock */
    printf("Do you want select tag before lock? Y/N");
    char resp[16];
    scanf("%s", resp);
    ...
    rfid_lock_t lock = {0};
    lock.mask =  0;
    lock.mask |= RFID_TAGOPS_MASK_ENABLE_TIMEOUT;
    lock.timeout = 2000;

    if (select_en) {
        lock.mask |= RFID_TAGOPS_MASK_ENABLE_SELECT;
        lock.select_en = SELECT_MASK_1;
    }

    lock.mask |= RFID_TAGOPS_MASK_ENABLE_PASSWD;
    lock.passwd = 0x00010011;

    lock.bank = UHF_LOCK_BANK_USER;
    lock.type = RFID_LOCK_TYPE_WRITE_WITH_PASSWD;
    
    status = device_rfid_lock_tag(dev, &lock);
    if (status != DEVICE_OK) {
        fprintf(stderr, "Failed to lock tag: 0x%02X\n", (int)status);
        device_destroy(dev);
        return 1;
    }
    printf("Tag locked successfully!\n");
    
    lock.bank = UHF_LOCK_BANK_USER;
    lock.type = RFID_LOCK_TYPE_WRITEABLE;
    
    status = device_rfid_lock_tag(dev, &lock);
    if (status != DEVICE_OK) {
        fprintf(stderr, "Failed to unlock tag: 0x%02X\n", (int)status);
        device_destroy(dev);
        return 1;
    }
